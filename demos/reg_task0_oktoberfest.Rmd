...
Do people drink more beer when they eat more chicken – or is it the other way around?
...
We split the data in advance using rsample's `initial_split` to create a random training and test data.

```{r}
library(rsample)

oktoberfest <- read_csv("data/oktoberfestgesamt19852024.csv")
colnames(oktoberfest) <- c("year","duration","visitors_total","visitors_daily",
                            "beer_price","beer_consumption","chicken_price","chicken_consumption")
set.seed(100)
data_split <- initial_split(oktoberfest, prop = 0.8)
train_oktoberfest <- training(data_split)
test_oktoberfest <- testing(data_split)
```
...
Apart from computational burden, we have seen one of them: Multicollinearity (high correlations destabilizing the model).
Other problem is the "[Curse of dimensionality](https://en.wikipedia.org/wiki/Curse_of_dimensionality)":
When the feature space gets larger, when more features join, surpassing the number of observations it becomes **high-dimensional**.
In this case, the model might struggle to identify meaningful patterns and trends from the data set.
This can cause the model to overfit or underfit.

One simple approach is to filter out features that have only one value or very few unique values.
This is called (near) zero-variance filtering.
Another advanced method would be a dimensionality reduction technique, which produces new predictors based on combinations or transformations of the original variables to reduce the feature space but retain most of the information.

We'll just remove the year and the duration, since it does not make sense to use them as explanatory variables if we want to model the relationship between the measured variables.

```{r remove-year-duration}
train_oktoberfest <- train_oktoberfest %>%
...
scaled_oktoberfest <- train_oktoberfest %>%
  mutate(across(!beer_consumption, ~ as.numeric(scale(.))))
```
...
We should **filter** and **scale** the predictor similarly.
Basically all the things we did to the training data set.
But there is a crucial catch! We should avoid treatments
that would cause **data leakage**, for example using mean of test set column, if we had done any imputation by taking column means or scaling test set with its own mean and standard deviation.

```{r}
# get mean and standard deviation of the training set
mean_sd <- train_oktoberfest %>%
  select(-beer_consumption) %>%
...
test_processed <- test_oktoberfest %>%
  select(-c(year, duration, beer_consumption)) %>%
  mutate(across(everything(),
    ~ (.- mean_sd[[paste0(cur_column(), "_mean")]]) /
...
Can you spot on a crucial mistake that we've made in the 

### Demo: Summarizing using tidymodels!
...
* workflows | step-wise pipelining
* tune | optimizing hyperparameters
...
test_oktoberfest <- testing(data_split)
          ...
          # determine your target and predictor(s)
recipe <- recipe(beer_consumption ~ ., data = train_oktoberfest) %>%
  update_role(year, new_role = "ID") %>%          # year can be used as an ID
  update_role(duration, new_role = "ignore") %>%  # do not use duration as a predictor
...
wf <- workflow() %>%
  add_model(linear_modeller) %>%
  add_recipe(recipe)

# Fitting on training set
fit <- wf %>% fit(data = train_oktoberfest)

# Prediction on test_oktoberfest
preds <- predict(fit, new_data = test_oktoberfest) %>%
  bind_cols(test_oktoberfest)

# evaluation
metrics <- preds %>%
  metrics(truth = beer_consumption, estimate = .pred, )
