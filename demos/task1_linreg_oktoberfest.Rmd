...
```{r}
oktoberfest_train <- read_csv("data/oktoberfest_train.csv")
oktoberfest_train
...
oktoberfest_train <- oktoberfest_train %>%
...
scaled_oktoberfest <- oktoberfest_train %>%
...
mean_sd <- oktoberfest_train %>%
  select(-beer_consumption) %>%
  mutate(across(everything(), as.numeric)) %>%
  summarise(across(everything(),
                   list(mean = mean, sd = sd))) 

oktoberfest_test <- read_csv("data/oktoberfest_test.csv")

test_processed <- oktoberfest_test %>%
...
rmse = sqrt(mean((oktoberfest_test$beer_consumption - predictions)^2))
rmse
```
...
test_set <- read_csv("data/oktoberfest_test.csv")

          # determine your target and predictor(s)
recipe <- recipe(beer_consumption ~ ., data = train_set) %>%
...
# determine modelling "machine"
linear_modeller <- linear_reg() %>% 
  set_engine("lm")
...
fit <- wf %>% fit(data = train_set)

# Prediction on test_set
preds <- predict(fit, new_data = test_set) %>%
  bind_cols(test_set)
